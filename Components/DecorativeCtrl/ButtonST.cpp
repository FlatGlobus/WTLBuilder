...
LRESULT CButtonST::OnCreate(LPCREATESTRUCT pCreateStruct)
{
	...
	m_bIsCheckBox = (nBS & 0x0000000F) == BS_CHECKBOX;
...
namespace DecorativeCtrl
{
   ...
   BOOL CButtonSTComp::CreateComponent(Component * _Parent)
   {
		...
		CWindowImpl<CButtonST, CButton, CButtonSTWinTraits>::Create((HWND)_Parent->GetHandle(),GetBoundsRect(),get_Caption(),get_InternalWindowStyle(),get_InternalWindowStyleEx());
...
*/
   ...
   LRESULT CButtonSTComp::OnSpecialPaint (UINT uMsg,WPARAM wParam, LPARAM lParam, BOOL &bHandled)
   {
