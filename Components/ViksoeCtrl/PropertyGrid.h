...
#define PGS_EX_DRAWFOCUS         0x00000080
...
#ifdef IDS_LASTVALUE
      TCHAR szText[128] = { 0 };
      ::LoadString(_Module.GetResouceInstance(), IDS_LASTVALUE, szText, (sizeof(szText)/sizeof(TCHAR))-1);
...
public:
   ...
   void DrawItem(CDCHandle dc, int iItem, int iSubItem, UINT iState)
   {
      ...
      if( (m_di.dwExtStyle & PGS_EX_DRAWFOCUS) && ((::GetFocus() == m_hWnd && (iState & CDIS_FOCUS)) || (dwStyle & LVS_SHOWSELALWAYS)) )
      {
...
{
    class CPropertyGridCtrlComp:public CPropertyGridCtrl,public ControlComponent<CPropertyGridCtrlComp>
    {
        
    public:
        CPropertyGridCtrlComp(LPTSTR _name);
        ~CPropertyGridCtrlComp(void);
        
        BEGIN_MSG_MAP(CPropertyGridCtrlComp)
            CHAIN_MSG_MAP(ControlComponent<CPropertyGridCtrlComp>)
        ...
        DECLARE_PROPERTY(TabNav,BOOL)
        DECLARE_PROPERTY(NoSheetNav,BOOL)
        DECLARE_PROPERTY(FullRowSel,BOOL)
        DECLARE_PROPERTY(InvertSel,BOOL)
        DECLARE_PROPERTY(AddItemAtend,BOOL)
        DECLARE_PROPERTY(DrawFocus,BOOL)*/
