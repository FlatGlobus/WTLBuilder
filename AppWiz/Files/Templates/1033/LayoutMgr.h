...
public:
	...
	struct	Constraint
		...
		Constraint()
			...
			::memset(_constraint, 0 , ATTACH_MAX*sizeof(SideConstraint));
			}
		};
...
inline void LayoutMgr<T>::DoLayout()
{
	...
	for (wgt = _constraints.begin(); wgt != _constraints.end(); wgt++)
		{
			CRect rc(wgt->second._pos.left - offset.x, wgt->second._pos.top - offset.y,
				wgt->second._pos.right,
				wgt->second._pos.bottom);
	...
	pT->Invalidate();
    //pT->RedrawWindow(NULL,NULL,RDW_INVALIDATE|RDW_UPDATENOW|RDW_ALLCHILDREN);
}

template <class T>
inline void LayoutMgr<T>::SetIgnoreFormSize(bool val)
{
	_ignoreFormSize=val;
}
